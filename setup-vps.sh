...
# Script pentru setup inițial VPS orex.site
# Acest script trebuie rulat pe VPS pentru prima dată
...
# Verifică dacă rulează pe VPS
if [ "$1" == "--local" ]; then
    echo "Rulând în modul local - va crea script pentru VPS"
    
    cat > vps_setup_remote.sh << 'VPS_SCRIPT'
#!/bin/bash
...
VPS_SCRIPT
    ...
    echo "Script pentru VPS creat: vps_setup_remote.sh"
    echo ""
    echo "Pentru a rula pe VPS:"
    echo "1. Copiază script-ul: scp vps_setup_remote.sh root@64.225.49.128:/tmp/"
    echo "2. Conectează-te: ssh root@64.225.49.128"
    echo "3. Rulează: chmod +x /tmp/vps_setup_remote.sh && /tmp/vps_setup_remote.sh"
    
    chmod +x vps_setup_remote.sh
    
else
    echo "Pentru a rula acest script, folosește:"
    echo "$0 --local"
    echo ""
    echo "Sau conectează-te direct la VPS și rulează comenzile manual."
fi